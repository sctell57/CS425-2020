...
namespace mathtool{
    ...
    public:
        ...
        Vector<T,3> from2dto3d() const;					//3d to 2d
        Vector<T,2> from3dto2d() const;					//3d to 2d

        Vector<T,3> from4dto3d() const;					//4d to 3d
        Vector<T,4> from3dto4d() const;					//3d to 4d
...
*/
    ...
    template<class T, int D >
    Vector<T,3> Vector<T,D>::from4dto3d() const
	{
		return Vector<T,3>(v[0]/v[3], v[1]/v[3], v[2]/v[3]);
	}

    template<class T, int D >
    Vector<T,2> Vector<T,D>::from3dto2d() const
	{
    	Vector<T,2> vector(v[0]/v[2], v[1]/v[2]);

    	return vector;
	}

    template<class T, int D >
    Vector<T,3> Vector<T,D>::from2dto3d() const
   	{
       	Vector<T,3> vector(v[0], v[1], 1);

       	return vector;
   	}

    template<class T, int D >
	Vector<T,4> Vector<T,D>::from3dto4d() const
	{
		return Vector<T,4>(v[0], v[1], v[2], 1);
	}
